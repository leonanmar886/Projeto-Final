...
if (strlen(nome)>50 || strlen(end)>100) {
    return -1;
  }
  return 1;
}
...
Passageiro* passageiro_novo(int id, char* nome, char* endereco){
  if (verifica( id, nome, endereco)==-1) return NULL;
...
int passageiro_igual(Passageiro* passa1, Passageiro* passa2) {
  ...
  if (passa1 == NULL || passa2 == NULL) {
    return -1;
  }

  if (passa1->id != passa2->id) {
    return 0;
  }

  if (strcmp(passa1->nome, passa2->nome) != 0) {
    return 0;
  }

  if (strcmp(passa1->endereco, passa2->endereco) != 0) {
    return 0;
  }
